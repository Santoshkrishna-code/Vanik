...
const createStockMovement = async (req, res) => {
    try {
        const validatedData = inventory_validator_1.stockMovementSchema.parse(req.body);
        const userId = req.user?.id;
        if (!userId) {
            res.status(401).json({ success: false, message: 'Unauthorized' });
            return;
        }
        const product = await prisma_1.default.product.findUnique({ where: { id: validatedData.productId } });
        if (!product) {
...
const getStockMovements = async (req, res) => {
    try {
        ...
        const [movements, total] = await Promise.all([
            ...
            prisma_1.default.stockMovement.count()
        ]);
        res.json({
            success: true,
            data: movements,
